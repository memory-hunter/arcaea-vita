...
std::map<std::string, jboolean> prefsBools;
std::map<std::string, jint> prefsInts;
std::map<std::string, jstring> prefsStrings;
std::map<std::string, jfloat> prefsFloats;
...
jfloat getFloatForKey(jmethodID, va_list args)
{
    ...
    double default_value = va_arg(args, double); // floats are promoted to double through varargs
    ...
    return it != prefsFloats.end() ? it->second : static_cast<float>(default_value);
}

void setFloatForKey(jmethodID, va_list args)
{
    jstring _key = va_arg(args, jstring);
    double val = va_arg(args, double); // floats are promoted to double through varargs
...
void flush(jmethodID, va_list args)
{
    l_debug("Flush called for preferences");
    savePrefs(shared_prefs_path);
    needsSaving.exchange(false);
}

void deleteValueForKey(jmethodID, va_list args)
{
    jstring _key = va_arg(args, jstring);
    ...
    {
        std::lock_guard<std::mutex> lock(prefsMutex);
        prefsBools.erase(key);
        prefsInts.erase(key);
        prefsStrings.erase(key);
        prefsFloats.erase(key);
    }
    needsSaving.exchange(true);
}
